...
display_help() {
    cat << EOF
Usage:
  hfd <REPO_ID> [--include include_pattern1 include_pattern2 ...] [--exclude exclude_pattern1 exclude_pattern2 ...] [--hf_username username] [--hf_token token] [--tool aria2c|wget] [-x threads] [-j jobs] [--dataset] [--local-dir path] [--revision rev] [--no-proxy]
...
Options:
  ...
  --no-proxy      (Optional) Bypass the proxy for Hugging Face API and file downloads.
...
# curl honors NO_PROXY/no_proxy, while aria2c and wget need an explicit option.
# Keep this opt-in so an existing proxy remains the default behavior.
if (( NO_PROXY_MODE )); then
    export NO_PROXY="${NO_PROXY:-*}"
    export no_proxy="${no_proxy:-$NO_PROXY}"
    ARIA2_EXTRA_ARGS+=(--no-proxy)
    WGET_EXTRA_ARGS+=(--no-proxy)
    printf "%bProxy bypass enabled (NO_PROXY=%s).%b\n" "$DIM" "$NO_PROXY" "$NC"
fi
...
# Keep the downloader and progress monitor in one cleanup path. Without this,
# Ctrl+C can stop the foreground shell while the background monitor keeps
...
trap 'printf "\n%bInterrupted. Stopping download and progress monitor.%b\n" "$YELLOW" "$NC"; exit 130' INT TERM
...
if [[ "$TOOL" == "aria2c" ]]; then
    ...
    wget -x -nH --cut-dirs="$CUT_DIRS" "${WGET_EXTRA_ARGS[@]}" ${HF_TOKEN:+--header="Authorization: Bearer $HF_TOKEN"} \
